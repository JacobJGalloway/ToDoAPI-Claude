...
namespace WarehouseInventory_Claude.Tests.Services
{
    public class ClothingServiceTests
    {
        ...
        private readonly ClothingService _service;
        ...
        public ClothingServiceTests()
        {
            ...
            _service = new ClothingService(_mockUoW.Object);
        }
        ...
        public async Task GetBySKUIdAsync_ReturnsItems()
        {
            ...
            var result = await _service.GetBySKUIdAsync("CLTH001");
        ...
        public async Task GetByLocationAndSKUAsync_ReturnsItems()
        {
            var items = new List<Clothing>
            {
                new() { PartitionKey = "WH001-CLTH001-a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6", SKUMarker = "CLTH001", LocationId = "WH001", UnloadedDate = DateTime.UtcNow }
            };
            _mockRepo.Setup(r => r.GetByLocationAndSKUAsync("WH001", "CLTH001")).ReturnsAsync(items);

            var result = await _service.GetByLocationAndSKUAsync("WH001", "CLTH001");
        ...
        public async Task UpdateBySKUIdAsync_CallsRepoAndSavesChanges()
        {
            ...
            await _service.UpdateBySKUIdAsync("CLTH001", item);

            _mockRepo.Verify(r => r.UpdateBySKUIdAsync("CLTH001", item), Times.Once);
            _mockUoW.Verify(u => u.SaveChangesAsync(), Times.Once);
        }
        ...
        {
            var partitionKey = "WH001-CLTH001-a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6";
        ...
        {
            var partitionKey = "WH001-CLTH999-ffffffffffffffffffffffffffffffff";
